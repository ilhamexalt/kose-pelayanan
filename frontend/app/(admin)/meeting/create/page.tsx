...
export default function CreateMeetingPage() {
  ...
  const [tanggal, setTanggal] = useState<Dayjs | null>(null);

  // Data tiruan untuk validasi
  const mockBookings = [
    {
      ruangan: "Pulau Sangiang",
      tanggal: dayjs().format("YYYY-MM-DD"), // Hari ini
      waktuMulai: "09:00",
      waktuSelesai: "10:00",
  ...
  const handleSave = () => {
    if (!ruang || !waktu || !waktu[0] || !waktu[1] || !tanggal) {
      messageApi.error("Harap lengkapi Ruang, Waktu Penggunaan, dan Tanggal!");
      return;
    }
    ...
    const conflict = mockBookings.find(b => 
      ...
      ((startTime >= b.waktuMulai && startTime < b.waktuSelesai) ||
       (endTime > b.waktuMulai && endTime <= b.waktuSelesai) ||
       (startTime <= b.waktuMulai && endTime >= b.waktuSelesai))
    );

    if (conflict) {
      messageApi.error(`Ruangan ${ruang} pada jam ${conflict.waktuMulai} - ${conflict.waktuSelesai} sudah diisi oleh ${conflict.instansi}.`);
      return;
    }
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-end items-center gap-4 mt-6 pb-12">
          <Button size="large" className="font-medium shadow-sm hover:opacity-90">
            Reset Form
          </Button>
          <Button size="large" type="primary" onClick={handleSave} style={{ backgroundColor: "#b22222", border: "none" }} className="font-medium shadow-sm hover:opacity-90">
            Simpan Jadwal
          </Button>
        </div>
