...
export async function POST(request: Request) {
  try {
    ...
    const { nama, kegiatan, tempat, linkZoom, tanggalMulai, tanggalSelesai, jamMulai, jamSelesai, status, createdBy } = data;
    ...
    return NextResponse.json({ success: true, message: 'Jadwal pimpinan berhasil ditambahkan' });
  } catch (error: any) {
    console.error('Error submitting jadwal pimpinan:', error);
...
export async function GET() {
  try {
    ...
    console.error('Error fetching jadwal pimpinan:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
